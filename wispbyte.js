...
const UUID = process.env.UUID || '020541d9-5557-4f14-8d73-c81058705bb8'; // 运行哪吒v1,在不同的平台需要改UUID,否则会被覆盖
...
const DOMAIN = process.env.DOMAIN || 'thor.pylex.software';       // 填写项目域名或已反代的域名，不带前缀，建议填已反代的域名
...
const PORT = process.env.PORT || 11869;                     // http和ws服务端口
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const vlessURL = `vless://${UUID}@thor.pylex.software:11869?encryption=none&sni=${DOMAIN}&type=ws&host=${DOMAIN}&path=%2F#${NAME}-${ISP}`;
